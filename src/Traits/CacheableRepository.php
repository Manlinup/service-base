...
trait CacheableRepository
{
    ...
    public function getCacheRepository()
    {
        if (is_null($this->cacheRepository)) {
            $this->cacheRepository = app(config('sak.cache.repository', 'cache'));
        }
    ...
    public function isSkippedCache()
    {
        ...
        $skipCacheParam = config('sak.cache.params.skipCache', 'skipCache');
    ...
    {
        $cacheEnabled = config('sak.cache.enabled', true);
        if (!$cacheEnabled) {
            return false;
        }
