...
abstract class BaseGenerator
{
    ...
    {
        $path = config('sak.generator.stubsOverridePath', __DIR__);
    ...
    {
        return config('sak.generator.rootNamespace', $this->getAppNamespace());
    }
    ...
    public function getConfigGeneratorClassPath($class, $directoryPath = false)
    {
        switch ($class) {
            case ('models' === $class):
                $path = config('sak.generator.paths.models', 'Models');
                break;
            case ('repositories' === $class):
                $path = config('sak.generator.paths.repositories', 'Repositories');
                break;
            case ('repository_interface' === $class):
                $path = config('sak.generator.paths.repository_interface', 'Repositories/Contracts');
                break;
            case ('base_repository_interface' === $class):
                $path = config('sak.generator.paths.base_repository_interface', 'Repositories/Contracts');
                break;
            case ('presenters' === $class):
                $path = config('sak.generator.paths.presenters', 'Presenters');
                break;
            case ('requests' === $class):
                $path = config('sak.generator.paths.requests', 'Http/Requests');
                break;
            case ('services' === $class):
                $path = config('sak.generator.paths.services', 'Services');
                break;
            case ('service_interface' === $class):
                $path = config('sak.generator.paths.service_interface', 'Services/Contracts');
                break;
            case ('transformers' === $class):
                $path = config('sak.generator.paths.transformers', 'Transformers');
                break;
            case ('validators' === $class):
                $path = config('sak.generator.paths.validators', 'Validators');
                break;
            case ('controllers' === $class):
                $path = config('sak.generator.paths.controllers', 'Http/Controllers/Api/V1');
                break;
            case ('base_controller' === $class):
                $path = config('sak.generator.paths.base_controller', 'Http/Controllers/Api/V1');
                break;
            case ('criteria' === $class):
                $path = config('sak.generator.paths.criteria', 'Criteria');
                break;
            case ('routes' === $class):
                $path = config('sak.generator.paths.routes', 'routes');
                break;
            case ('base_routes' === $class):
                $path = config('sak.generator.paths.base_routes', 'routes');
                break;
            default:
                $path = '';
        }
